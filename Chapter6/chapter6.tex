...
\subsection{Assumption \ref{assumpLowerBound}: integrable functions } \la{sec:6:consistencyIntegrable}
...
\subsection{Assumption \ref{assumpLowerBound}: beyond integrable functions} \la{sec:6:consistencyNonIntegrable}
...
\begin{thm} \la {thmHomoConsistency} Suppose Assumption \ref{assumpHomo} holds. Suppose that there exists a continuous Gaussian process $G$ such that $x_{[nt], n} \Rightarrow G(t)$, on $D[0,1]$, where $x_{i,n} = x_i / d_n$  and $0 < d_n \to \infty$ is a sequence of real numbers.
...
Assumption \ref{assumpMartingale} ensures the model (\ref {intro.eqn1}) to have a martingale structure.
...
\begin{thm} \la {endo13}Suppose that Assumption \ref {4.2} holds and in addition to Assumption \ref{assumpHomo}, for any $\theta\not= \theta_0$,
...
\begin{figure}[hb]
  \centering
  \includegraphics[width=15cm]{theta_density_v3_horizontal}
  \caption{Density estimates of $\hat{\theta}_n$.}\la{theta_density_horizontal}
\end{figure}
...
\includegraphics[width=15cm]{beta_1_t_ratio}
...
\begin{figure}[hb]
  \centering
\includegraphics[width=15cm]{GDP_plot}
\caption{Plots of log(CO$_2$) against log(GDP) for Belgium, Denmark and France.}\la{fig.co2gdp}
\end{figure}
...
\begin{figure}[hb]
  \centering
  \includegraphics[width=16cm]{b_1_plot_v2} \\
\caption{Estimates and 95\% Confidence Intervals of $\hat{\beta}_{1n}$.}\la{beta1_plot}

\end{figure}

\begin{figure}[hb]
  \centering
  \includegraphics[width=16cm]{b_2_plot_v2} \\
  \caption{Estimates and 95\% Confidence Intervals of $\hat{\beta}_{2n}$.}\la{beta2_plot}
\end{figure}
...
\begin{lem} \la{lemMarkov} Let $x_t$ be defined as in Example 2. For any $g$ such that $\int_{-\infty}^{\infty} |g(x)| \pi(dx) < \infty$ and $\int_{-\infty}^{\infty} g(x) \pi(dx) \ne 0$, we have
\begin{align}
...
\begin{lem} \la{lemLagJoint} Under Assumption \ref {4.1}, for any bounded $g_i, i=1,2,$
satisfying $\int_{-\infty}^{\infty} |g_i(x)|dx < \infty$ and $\int_{-\infty}^{\infty} g_i(x) dx  \ne 0$, we have
...
\begin{lem} \la{lemEndo} Under Assumption \ref{4.2}, for any twice continuous differentiable function $g_i$, $i = 1, 2$, satisfying $|g_i'(x)| \le K (1 + |x|^{\al})$ for some constants $K, \al > 0$ for all $x \in R$, and if $q \ge \max(3, 2\al)$ where $q$ is given Assumption \ref{4.2}, we have
...
\be
\sum_{t = 1}^n (f(x_t, \pi_0) - f(x_t, \theta_0)) u_t = o_P\big [n\, v(\sqrt{n})\big ]. \la {999}
\ee
\end{lem}
...
P \Big ( \inf_{\theta \in \Theta \cap \mathcal N^c}\, D_n(\theta, \theta_0) \ge  \kappa^2_n /M_1 \Big ) \ge 1 -3 \eta,
\ee
for all $n > n_0$.
...
&\ge &\inf_{1\le j\le m_0}P\Big (D_n(\pi_j, \theta_0) \ge \kappa^2_n/(2m_0M_1) \Big )-\eta \ge 1-3\eta,
\eestar
...
This, together with (\ref {21}) and (\ref {98}), implies that,
 for any $\eta>0$, there exists $n_0 > 0$ and $M_0 > 0$ such that for all $n > n_0$ and $M\ge M_0$,
\begin{align}
P\Big ( D_n(\pi_0, \theta_0) \ge \kappa^2_n M^{-1} \Big ) &\ge P \Big ( \sum_{t = 1}^n ( f(x_t, \pi_0) - f(x_t, \theta_0))^2 \ge \kappa^2_n\, M^{-1}/2 \Big ) - \eta \no\\
&\ge1 -2 \eta, \no
\end{align}
as required.
...
\begin{proof}[Proof of Theorem \ref {thmMarkovConsistency}]
 (\ref {20}) follows from (\ref {ty}) of  Lemma \ref{lemMarkov} with $g(x)=T(x)+T^2(x)$.
  (\ref {21}) follows from (\ref {ty10}) of Lemma \ref{lemMarkov} with   $g(x)=(f(x, \theta)-f(x, \theta_0))^2$ and the facts that
   $\int_{-\infty}^{\infty} (f(s, \theta) - f(s, \theta_0))^2 \pi(ds) > 0$, for any $\theta \ne \theta_0$.
\end{proof}
...
\subsection{A Unit Root Test} \la{sec.unitRootTest}
...
\label{unit_root_table} \center{
\begin{tabular}{|c | c | c |}
\hline
Countries & DF for $\log($GDP$)$ & DF for $\log($CO$_2)$ \\
\hline
 AUS & 4.9129 & 2.0222 \\
 AUT & 3.6022 & 0.7622 \\
 BEL & 4.8375 & -0.2559 \\
...
$ \hat{\theta}_n $  & 200   &0.10018 & (0.00121) & & 0.10018 & (0.00125) & & 0.10011 & (0.00139)\\
 & 500 & 0.10014 & (0.00074) & & 0.10013 & (0.00077) & & 0.10004 & (0.00086) \\
...
$ \hat{\theta}_n $  & 200   &0.10014 & (0.00131) & & 0.10024 & (0.00136) & & 0.10014 & (0.00146)  \\
 & 500 &0.10009 & (0.00081) & & 0.10017 & (0.00083) & & 0.10012 & (0.00092)  \\
...
$ \hat{\theta}_n $  & 200   &0.10033 & (0.00192) & & 0.10017 & (0.00202) & & 0.10026 & (0.00223)  \\
 & 500 & 0.10022 & (0.00122) & & 0.10020 & (0.00124) & & 0.10012 & (0.00138) \\
...
\begin{figure}[hb]
  ...
  \includegraphics[width=15cm]{theta_density_v3}\la{theta_unscaled}
\end{figure}

\begin{figure}[hb]
  \centering
  \caption{Density estimate of $\hat{\al}_n$.}
  \includegraphics[width=15cm]{alpha_density_unscaled}
\end{figure}

\begin{figure}[hb]
  \centering
  \caption{Density estimate of $\hat{\beta}_{1n}$.}
  \includegraphics[width=15cm]{beta1_density_unscaled}
\end{figure}

\begin{figure}[hb]
  \centering
  \caption{Density estimate of $\hat{\beta}_{2n}$.}
  \includegraphics[width=15cm]{beta2_density_unscaled}\la{beta2_unscaled}
\end{figure}

\begin{figure}[hb]
  \centering
  \caption{Density estimate of $\hat{\theta}_n$ $t$-ratios.}
  \includegraphics[width=15cm]{theta_t_ratio}\la{theta_t}
\end{figure}

\begin{figure}[hb]
  \centering
  \caption{Density estimate of $\hat{\al}_n$ $t$-ratios.}
  \includegraphics[width=15cm]{alpha_t_ratio}
\end{figure}

\begin{figure}[hb]
  \centering
  \caption{Density estimate of $\hat{\beta}_{1n}$ $t$-ratios.}
  \includegraphics[width=15cm]{beta_1_t_ratio_long}
\end{figure}

\begin{figure}[hb]
\centering
  \caption{Density estimate of $\hat{\beta}_{2n}$ $t$-ratios.}
  \includegraphics[width=15cm]{beta_2_t_ratio}\la{beta2_t}
\end{figure}

\begin{figure}[hb]
  \centering
  \caption{Estimates and 95\% Confidence Intervals of $\hat{\al}_{n}$.}
  \includegraphics[width=16cm]{a_plot_v2}\la{al_fig} \\
\end{figure}

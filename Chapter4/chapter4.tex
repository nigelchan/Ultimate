...
\begin{assump} \ref{assump:4:kernelLocal} The kernel $K$ has a compact support and is twice continuously differentiable on $R$, satisfying that $\int x K(x) dx=0$ and $\int y^{2}K(y)dy\not=0.$
\end{assump}
...
\begin{rem} \la{bias} If incorporating a bias term in (\ref{q1}), under the conditions of Theorem \ref {thm:4:localLinear},
...
\begin{lem} \la {lem3} Let $K_1(x)=xK(x)$. Under Assumption 4.1, we have

...
\be
  S_n(x) =\sum_{k=1}^n u_k f[(x_k+x)/h], \quad x\in R^d,\la {eqn:1:in1}
\ee
...
\begin{assump} \la{assump:1:highLevel}
  ...
  \be
    \sup_{\|x\|\le b_n} \sum_{t=1}^n f^2[(x_t+x)/h] &=&O_P(c_n). \la {eqn:1:m1}
  \ee
\end{assump}
...
\begin{thm} \la {thm:1:th4} Under Assumptions \ref{assump:1:martingale}-\ref{assump:1:bandwidth}, we have
  ...
  \be
    \sup_{\|x\|\le b_n} \sum_{t=1}^n f^2[(x_t+x)/h] &=& O(c_n), \quad  a.s., \la {eqn:1:m1a9}
  \ee
...
\be
\sup_{|x|\le n^m} \sum_{k=1}^n f^2[(x_k+x)/h] =O_P[a(n)\, h]. \la {eqn:1:1.2}
\ee
...
\be
\inf_{|x|\le b_n+1}  \sum_{k=1}^n E f^2[(x_k+x)/h]\ge a(n)\, h/C_0,  \la {eqn:1:wond}
\ee
then, for any  $h>0$ satisfying $n^{-\ep_0}a(n)\, h\to \infty$ for some $\ep_0>0$, we have
\be
\Big\{\inf_{|x|\le b_n} \sum_{k=1}^n f^2[(x_k+x)/h]\Big\}^{-1} =O_P\big\{[a(n)\, h]^{-1}\big\}. \la {eqn:1:1.2a}
\ee
\end{thm}
...
\be
P\Big(\inf_{|x|\le b_n} \sum_{k=1}^n f^2[(x_k+x)/h] \ge a(n)\, h/C_\eta \Big) \ge 1-\eta. \la {eqn:1:1.5}
\ee
...
\be
\sup_{|x|\le n^m} \sum_{k=1}^n f^2[(x_k+x)/h] &=&O_P(\sqrt n\, h), \la {eqn:1:1.2a1}
\ee
for any integer $m>0$, and
\be
\Big\{\inf_{|x|\le M_0\sqrt n} \sum_{k=1}^n f^2[(x_k+x)/h]\Big\}^{-1} &=& O_P\big\{(\sqrt n\, h)^{-1}\big\}, \la {eqn:1:m15}
\ee
for a fixed $M_0>0$.
\end{cor}
...
\be
 \frac{1}{\sqrt nh} \sum_{t=1}^{n} f^2 [(x_t + y_n )/h]&\to_D&\int f^2(s) ds\, L_W(1,y), \la {eqn:1:ez1}
\ee
...
\be
 \frac{1}{\sqrt nh} \sum_{t=1}^{n} f^2 [(x_t + y )/h]&\to_P&\int f^2(s) ds\, L_W(1,0), \la {eqn:1:ez3}
\ee
...
\big[|u_t-u_t'|+E(|u_t-u_t'|\mid {\mathcal F}_{t-1})\big]\,f[(x_{t}+y_j)/h]\big| \no\\
...
\bestar
\max_{0\le j\le m_n}\, \sum_{t=1}^{n}\,f^2[(x_{t}+y_j)/h]E [(u_t^*)^2\mid {\mathcal F}_{t-1}]=O_P(c_n).
\eestar
For any $\eta>0$, there exists a $M_0>0$ such that
\bestar
P\big(\max_{0\le j\le m_n}\, \sum_{t=1}^{n}\,\si_t^2\ge M_0  c_n)\le \eta.
\eestar
where $\si_t^2= f^2[(x_{t}+y_j)/h]E [(u_t^*)^2\mid {\mathcal F}_{t-1}]$, whenever $n$ is sufficiently large.
...
&\le&
 ...
 f[(x_{k}+y_j)/h]\ge M_0 (c_n\,\log n )^{1/2}, \ \ \sum_{t=1}^{n}\,\si_t^2
 \le M_0\, c_n \Big] +\eta\no\\
...
\bestar
\max_{0\le j\le m_n}\, \sum_{t=1}^{n}\,f^2[(x_{t}+y_j)/h]E [(u_t^*)^2\mid {\mathcal F}_{t-1}]=O(c_n), \quad a.s.
\eestar
...
&\le & O_P\big[(c_n \log n)^{1/2}\big]+\sup_{\|x\|> b_n/2} |f(x/h)|\sum_{t=1}^{n}\,|u_t|\no\\
...
&=&b\,h\, \int_{-\infty}^{\infty} f^2(s)\, p(-x+sh) ds \le C^*\, h, \la {eqn:1:45}
\ee
for any $x\in R$ and
\be
...
{\bf Proof.} Only consider  $Z_k^*(x)=Z_k(x)$, as the situation that $Z_k^*(x)=Z_{kn}(x)$ is similar.
...
We are now ready to prove (\ref {eqn:1:1.2}) and (\ref {eqn:1:1.2a}) under the additional condition (\ref {eqn:1:1}).
 ...
 (\ref {eqn:1:1.2}) first. As in proof of  (\ref {eqn:1:p6}) and (\ref {eqn:1:p7}), but letting $ y_j=-[n^m]-1+j\,/ m_n',j=0, 1,2,...,\,m_n, $ where $m_n'=[nh^{-2}/a(n)]$ and $m_n=2([n^m]+1)m_n'$,  we have
  \be
 \sup_{|x|\le n^m} \sum_{k=0}^n f^2[(x_k+x)/h] &\le & \max_{0\le j\le m_n}\sum_{k=0}^n f^2[(x_k+y_j)/h]+ C\,a(n)\, h. \la {eqn:1:fi1}
  \ee
Note that
\bestar
...
\bestar
 \inf_{0\le j\le m_n} E Z_{ 1n}(y_j)  &=& \frac {1}{d_n}  \inf_{0\le j\le m_n} E\sum_{i=1}^{d_n} Z_{ in}(y_j) =  \frac {1}{d_n} \inf_{0\le j\le m_n} E\,\sum_{t=1}^{\rho_{d_n}\wedge n}\,f^2[( x_t+y_j)/h] \no\\
&\ge &\frac {1}{d_n} \inf_{0\le j\le m_n}E\,\Big(\sum_{t=1}^{ n}\,f^2[( x_t+y_j)/h] -I(\rho_{d_n} \le n) \sum_{t=1}^{ \rho_{d_n}}\,f^2[( x_t+y_j)/h]\Big)\no\\
&\ge &\frac {1}{d_n}\big (\inf_{|x|\le b_n+1}E\sum_{k=1}^nf^2[( x_t+x)/h]-M_n\big)\no\\
...
\medskip
\noindent {\bf Proof of Corollary \ref {cor:1:th3}.}
...
&\ge &\frac {h}{\sqrt k}\inf_{|x|\le M_0\sqrt n+1}\int_{-\infty}^{\infty} f^2(y)\, p_k[(yh-x)/\sqrt k]dy \no\\
&\ge&\frac {h}{\sqrt k}\inf_{|x|\le 3 M_0} p_k(x)\int_{|y|\le M_1} f^2(y)dy \no\\
&\ge&\frac {A_0 h}{2\sqrt n}\, \int_{|y|\le M_1} f^2(y)dy,
\eestar

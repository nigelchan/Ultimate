...
we have
\be \la{th1.eqn1}
\sup_{x\in R} \Big |\frac{c_n}{n}S_{2n}(x)- \tau\, L_{G_n}(1, -x)\Big| &=&o_P(\log^{-\beta} n),
\ee
...
\begin{rem} \la{rem:3:optimal} Corollary \ref {cor1} essentially improves Theorem 3.2 of
...
\be
\big[\inf_{|x|\le r_n} | S_{2n}(x)|\big]^{-1}=O_P(c_n / n), \la {78}
 \ee
...
\be \la{th10}
\sup_{x\in R} \Big |\frac {d_n}{nh}S^*_{2n}(x)- \tau\, L_{Y_n}(1, -x)\Big| &=&o_P(\log^{-\beta} n),
\ee
...
\end{rem}
\begin{prop} \la{prop1}   On a rich probability space, there exists  a fraction Brownian motion $\{W_{1 - \mu}(t), 0\le t < \infty\}$ such that
\be \la{prop1.eqn2}
...
\subsection{Preliminary Lemmas} \la{section:3:lemma}
...
\begin{lem} \la{lemLocal} For any $s, t \in R$ and $\xi > 0$, there exists a constant $C$ such that
\be
...
\be
 \max_{1 \le i \le q_{n1}} \max_{1 \le j \le q_{n2}}\Big(| \sum_{\substack{w = 2 \\ w \in even}}^{T_n} \Delta_{nw}(t_i, s_j)|+| \sum_{\substack{w = 2 \\ w \in odd}}^{T_n} \Delta_{nw}(t_i, s_j)|\Big)  &=& O_{a.s.} (\eta_n \, \log n).  \la {21}
\ee
...
\subsection{Proof of Theorem \ref{th1}} \la{section:3:proof}
...
as $n^{\zeta} / c_n \le n^{-\de}$ and $\rho > \de / (1 - \zeta)$.
 ...
 Recall $\{G_n(t); 0 \le t \le 1\} =_D \{G(t); 0 \le t \le 1\}$ for all $n\ge 1$,  by Assumption 2.2. For any $\ep>0$ and $\beta>0$, we have
 \bestar

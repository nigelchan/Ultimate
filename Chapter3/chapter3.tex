...
\bestar
&& \E (  f_{t,s}(x_{k,n})\mid {\mathcal F}_{n, j}) \no\\
...
\bestar
|\E (  f_{t,s}(x_{k,n})\mid {\mathcal F}_{n, j})| & \le& C\, c_n^{-1}\,  d_{k,j, n}^{-1} \min\{|t - s|c_n^{-1} d_{k,j, n}^{-1}, 1\}\no\\
...
\bestar
&& \E (  f^2_{t,s}(x_{k,n})\mid {\mathcal F}_{n, j}) \le C\,
 \E (  |f_{t,s}(x_{k,n})|\mid {\mathcal F}_{n, j}) \no\\
&&\quad =\ C\,
...
\begin{lem} \la{lemLocal} For any continuous Gaussian process $G$ with covariance function satisfying
 \be
E G(t) G(s) = \frac{c}{2} \{ |t|^{2w} + |s|^{2w} - |t - s|^{2w} \},
\ee
...
\subsection{Proofs of theorems} \la{section:3:proof}
